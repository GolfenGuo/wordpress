...
echo "========================================================================"
echo "      Database Host Address:  $MYSQL_HOST"
echo "      Database Port number:   $MYSQL_PORT"
...
for ((i=0;i<10;i++))
do
    DB_CONNECTABLE=$(mysql -u$MYSQL_USERNAME -p$MYSQL_PASSWORD -h$MYSQL_HOST -P$MYSQL_PORT -e 'status' >/dev/null 2>&1; echo "$?")
...
if [[ $DB_CONNECTABLE -eq 0 ]]; then
    DB_EXISTS=$(mysql -u$MYSQL_USERNAME -p$MYSQL_PASSWORD -h$MYSQL_HOST -P$MYSQL_PORT -e "SHOW DATABASES LIKE '"$MYSQL_INSTANCE_NAME"';" 2>&1 |grep "$MYSQL_INSTANCE_NAME" > /dev/null ; echo "$?")

    if [[ DB_EXISTS -eq 1 ]]; then
        echo "=> Creating database $MYSQL_INSTANCE_NAME"
        RET=$(mysql -u$MYSQL_USERNAME -p$MYSQL_PASSWORD -h$MYSQL_HOST -P$MYSQL_PORT -e "CREATE DATABASE $MYSQL_INSTANCE_NAME")
        if [[ RET -ne 0 ]]; then
        ...
        if [ -f /initial_db.sql ]; then
            ...
            RET=$(mysql -u$MYSQL_USERNAME -p$MYSQL_PASSWORD -h$MYSQL_HOST -P$MYSQL_PORT $MYSQL_INSTANCE_NAME < /initial_db.sql)
            if [[ RET -ne 0 ]]; then
